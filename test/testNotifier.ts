...
const testFixtures = (() => {
  const sendMail: MyMailApp = {
    sendEmail(recipient, subject, text, options) {
      console.log(`To: ${recipient}`);
      console.log(`From: ${options.from}`);
      console.log(`Reply-to: ${options.noReply}`);
      console.log(`subject: ${subject}`);
      console.log(`html: ${options.htmlBody}`), console.log(`text: ${text}`);
      return this;
    },
    getDrafts() {
      return new Array<GoogleAppsScript.Gmail.GmailDraft>();
    },
  };
...
describe('Notifier tests', () => {
  it('should log a success', () => {
    ...
    expect(actual).to.deep.equal([
      {input: testFixtures.txn1, member: testFixtures.member1},
    ]);
  });
  it('should log a failure', () => {
    ...
    expect(actual).to.deep.equal([
      {
        input: testFixtures.txn1,
        member: testFixtures.member1,
        error: testFixtures.error,
      },
    ]);
  });
