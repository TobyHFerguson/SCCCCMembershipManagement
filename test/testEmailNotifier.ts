...
describe('Email Notifier tests', () => {
  ...
  it('should send an email to the member on success, and a copy to the bcc list', () => {
    ...
    expect(stub.sendEmail, 'subject didnt match').to.be.calledWith(
      testFixtures.txn1['Email Address'],
      'Thanks for joining SCCCC',
      'joinSuccessSubject: PLAIN',
      Sinon.match(options)
    );
    expect(stub.sendEmail, 'email didnt match').to.be.calledWith();
    //   expect(stub.sendEmail, 'plain body didnt match').to.be.calledWith('joinSuccessSubject: PLAIN');
    //   expect(stub.sendEmail, 'options didnt match').to.be.calledWith(options);
