const testFixtures = (() => {
    ...
    sendMail: (recipient, subject, text, options) => {
  ...
  function testEmailNotificationJoinFailure() {
    const templates = new Templates(GmailApp.getDrafts(), testFixtures.subject_lines)
    const notifier = new EmailNotifier(templates, { test: true, mailer: { sendMail: testFixtures.sendMail }})
