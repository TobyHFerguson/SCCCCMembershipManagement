...
declare namespace bmPreFiddler {
  ...
  interface Fiddler {
    ...
    dumpValues(): {};
    mapRows(
      func: (row: object, properties: {rowFormulas: object}) => object
    ): Fiddler;
...
interface NotificationType {
  ...
  renewalFailure(txn: Transaction, member: Member, error: Error): void;
...
interface UsersCollectionType {
  get(userKey: string): GoogleAppsScript.AdminDirectory.Schema.User;
  get(
    userKey: string,
    options: object
  ): GoogleAppsScript.AdminDirectory.Schema.User;
  insert(user: UserType): GoogleAppsScript.AdminDirectory.Schema.User;
  list(optionalArgs: object): GoogleAppsScript.AdminDirectory.Schema.Users;
...
interface EmailConfigurationCollection {
  ...
  expirationNotification: EmailConfigurationType;
