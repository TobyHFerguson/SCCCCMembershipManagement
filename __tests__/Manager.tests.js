...
describe('Manager tests', () => {
  ...
  describe('processPaidTransactions_', () => {
    ...
    describe('sending emails', () => {
      it('should send an email when a member is added', () => {
        ...
        expect(sendEmailFun).toHaveBeenCalledWith({
          ...
          htmlBody: Manager.expandTemplate(actionSpecByType.get('Join').Body, members[0])
        });
      ...
      it('should send an email when the member is renewed', () => {
        ...
        expect(sendEmailFun).toHaveBeenCalledWith({
          ...
          htmlBody: Manager.expandTemplate(actionSpecByType.get('Renew').Body, members[0])
        });
        expect(sendEmailFun).toHaveBeenCalledTimes(1);
      });
    })
    describe('membership expiry period tests', () => {
      it('if renewal is before expiry then new expiry is  old expiry + period', () => {
        const members = [{ Email: "test1@example.com", Period: 1, First: "John", Last: "Doe", Joined: Manager.today(), Expires: Manager.addDaysToDate(Manager.today(), 10), "Renewed On": "" },]
        const expectedMembers = [
          { Email: "test1@example.com", Period: 1, First: "John", Last: "Doe", Joined: getDateString(), Expires: Manager.addDaysToDate(new Date(), 365 + 10), "Renewed On": Manager.today() },
        ]
      ...
      it('if renewal is after expiry then new expiry is today + period', () => {
        const members = [{ Email: "test1@example.com", Period: 1, First: "John", Last: "Doe", Joined: Manager.today(), Expires: Manager.addDaysToDate(Manager.today(), -10), "Renewed On": "" },]
        const expectedMembers = [
          { Email: "test1@example.com", Period: 1, First: "John", Last: "Doe", Joined: getDateString(), Expires: Manager.addDaysToDate(new Date(), 365), "Renewed On": Manager.today() },
        ]
        Manager.processPaidTransactions(transactionsFixture.paid, members);
        members.forEach(e => { e.Joined = getDateString(e.Joined); e.Expires = getDateString(e.Expires) });
        expectedMembers.forEach(e => { e.Joined = getDateString(e.Joined); e.Expires = getDateString(e.Expires) });
  ...
  describe('addDaysToDate  ', () => {
    it('should add a number of days to a date', () => {
      ...
      const result = Manager.addDaysToDate(date, 2);
    ...
    it('should work with zero', () => {
      const date = new Date('2021-01-01');
      const result = Manager.addDaysToDate(date, 0);
  ...
  describe('actionSchedule', () => {
    it('should create an actionSchedule', () => {
      ...
      const expected = [
        { Email: txn["Email Address"], Type: Manager.ActionType.Expiry1, Date: Manager.addDaysToDate(today, 365 + O1) },
        { Email: txn["Email Address"], Type: Manager.ActionType.Expiry2, Date: Manager.addDaysToDate(today, 365 + O2) },
        { Email: txn["Email Address"], Type: Manager.ActionType.Expiry3, Date: Manager.addDaysToDate(today, 365 + O3), },
    ...
    it('should update an existing actionSchedule', () => {
      const members = [{ Email: "test1@example.com", Period: 1, first: "John", last: "Doe", Joined: '2021-01-01', Expires: Manager.addDaysToDate(today, 365) }];
      const actionSchedule = [
        { Email: "test1@example.com", Type: Manager.ActionType.Join, Date: today, },
        { Email: "test1@example.com", Type: Manager.ActionType.Expiry1, Date: Manager.addDaysToDate(today, 365 - O1) },
        { Email: "test1@example.com", Type: Manager.ActionType.Expiry2, Date: Manager.addDaysToDate(today, 365 - O2) },
        { Email: "test1@example.com", Type: Manager.ActionType.Expiry3, Date: Manager.addDaysToDate(today, 365 - O3), },
        { Email: "test1@example.com", Type: Manager.ActionType.Expiry4, Date: Manager.addDaysToDate(today, 365 + 1), }
